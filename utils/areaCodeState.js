...
// US area code (NPA) → state. Mirrors how Ringba derives InboundNumber:State
// from the caller's number, so SalesRadix gets the same State value.
const STATE_NPAS = {
...
const NPA_STATE = {};
for (const [st, npas] of Object.entries(STATE_NPAS)) {
  for (const npa of npas) NPA_STATE[String(npa)] = st;
}

// Best-effort 2-letter state for a 10-digit (or 11-digit) US number.
function stateForNumber(num) {
  const d = String(num || "").replace(/\D/g, "");
  let ten = d;
  if (d.length === 11 && d[0] === "1") ten = d.slice(1);
  if (ten.length !== 10) return "";
  return NPA_STATE[ten.slice(0, 3)] || "";
}

module.exports = { stateForNumber, NPA_STATE };
