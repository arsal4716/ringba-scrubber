...
/**
 * ✅ Snapshot replace:
 * - removes ALL previous records for campaignName+fetchType
 * - inserts the newly fetched set
 *
 * This makes the Call table always represent the "latest run"
 */
async function replaceCallsSnapshot({ campaignName, fetchType, numbers, runId }) {
  ...
  // If API returned 0 numbers, you probably still want snapshot to be empty:
  // delete previous and return (so UI shows 0)
...
const executeFetch = async (timezone) => {
  ...
  try {
    ...
    for (const t of tasks) {
      logger.info(`Fetching ${t.campaignName} (${t.fetchType})`);
      ...
      try {
        ...
        logger.error(`Failed to fetch ${t.campaignName} (${t.fetchType}): ${err?.message || err}`);
      }
    ...
    for (const r of results) {
      ...
      perCampaignStats.push({
        campaignName: `${r.campaignName}${r.campaignName === "SSDI" ? ` (${r.fetchType})` : ""}`,
        fetchedCount,
      ...
      // File planning (merge SSDI & Medicare)
    ...
    // 3) Generate files
    const fileDocs = await fileService.generateFiles(dateStr, Array.from(filePlans.values()));

    // SSDI combined stats line (optional)
    ...
    logger.info(`Fetch job completed successfully. Files generated: ${fileDocs.length}`);
  } catch (err) {
